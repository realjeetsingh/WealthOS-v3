...
export const searchSymbols = async (query: string, isCrypto = false): Promise<SymbolResult[]> => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // Pass through specific error types if defined, otherwise use status-based type
      const errorMsg = data.type || (response.status === 429 ? 'RATE_LIMIT' : 'API_FAILURE');
    ...
    const knownErrors = ['RATE_LIMIT', 'AUTH_FAILURE', 'API_FAILURE'];
    if (knownErrors.includes(error.message)) {
      throw error;
    }
    // For anything else, return a generic but identifiable error
...
export const fetchCryptoPrice = async (symbol: string): Promise<number | null> => {
  ...
  try {
    ...
    const response = await fetchWithTimeout(`${API_BASE_URL}/crypto/quote?symbol=${s.toUpperCase()}`, { timeout: 8000 });
...
export const fetchMarketPrice = async (symbol: string, isCrypto = false): Promise<number | null> => {
  ...
  try {
    ...
    const response = await fetchWithTimeout(`${API_BASE_URL}/quote?symbol=${encodedSymbol}`, { timeout: 8000 });
