    ...
    clusters.forEach(cluster => {
      ...
      if (cluster.txs.length >= 2) {
        const sortedDates = cluster.txs
          .map(t => new Date(t.date || t.timestamp.toDate()).getTime())
          .sort((a, b) => a - b);
      ...
      if (score >= 50) {
        const sortedByDate = [...cluster.txs].sort((a, b) => 
          new Date(b.date || b.timestamp.toDate()).getTime() - 
          new Date(a.date || a.timestamp.toDate()).getTime()
        );
        ...
        suggestions.push({
          ...
          date: sortedByDate[0].date || sortedByDate[0].timestamp.toDate().toISOString().split('T')[0],
