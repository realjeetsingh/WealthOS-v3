...
const Signup: React.FC = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (isNewUser) {
        trackEvent('google_signin_new_user');
        await recordLegalConsent(user.uid);
      } else {
        trackEvent('google_signin_existing_user');
      }
      ...
      } else if (err?.code === 'auth/network-request-failed' || err?.message?.toLowerCase().includes('network') || err?.message?.toLowerCase().includes('internet')) {
  ...
  return (
    <motion.div 
      ...
      className="min-h-dvh flex flex-col md:flex-row bg-white selection:bg-indigo-100"
    >
      ...
      <div className="flex-1 flex flex-col items-center justify-start p-8 md:p-16 pt-12 pb-10 bg-gray-50 relative min-h-dvh overflow-y-auto w-full">
              ...
              {error === "Unable to connect. Please check your internet connection." && (
                <div className="ml-8">
                  <button
                    ...
                    className="text-xs font-black text-red-800 hover:underline flex items-center gap-1 cursor-pointer"
                  >
                    Retry Connection
                  </button>
                </div>
