                ...
                >
                  ...
                  <div className="flex flex-col gap-3 pt-4">
                    ...
                    <div className="pt-4 border-t border-gray-100">
                        {/* Hidden check for production */}
                        <label className="hidden flex items-center justify-between cursor-pointer group">
                             <div className="flex items-center gap-2">
                                <div className="p-1.5 bg-gray-100 rounded-lg group-hover:bg-gray-200 transition-colors">
                                    <ShieldCheck className="w-3.5 h-3.5 text-gray-500" />
                                </div>
                                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Transaction Analysis</span>
                             </div>
                             <input 
                                type="checkbox" 
                                checked={isDebugMode} 
                        ...
                        {selectedDetailTransaction.rawSMS && (
                            <motion.div 
                                initial={{ opacity: 0, height: 0 }}
                                animate={{ opacity: 1, height: 'auto' }}
                                className="mt-4 p-4 bg-gray-50 border border-gray-100 rounded-2xl overflow-hidden text-left"
                            >
                                <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-3">Parsed Transaction Source</p>
                                <div className="space-y-4 text-left">
                                    <div>
                                        <p className="text-[8px] font-bold text-gray-400 uppercase tracking-tighter mb-1">Raw SMS Message</p>
                                        <p className="text-xs text-gray-600 leading-relaxed italic bg-white p-3 rounded-xl border border-gray-100 break-words font-sans">
                                            "{selectedDetailTransaction.rawSMS}"
                                        </p>
                                    </div>
                                    {selectedDetailTransaction.categoryConfidence && (
                                        <div className="flex justify-between items-center pt-3 border-t border-gray-100">
                                            <div className="text-[10px] font-bold text-gray-500 uppercase tracking-tight flex items-center gap-1.5">
                                                <BrainCircuit className="w-4 h-4 text-[#6334FD]" />
                                                Classification Confidence
                                            </div>
                                            <div className="text-xs font-black text-gray-900 bg-indigo-50 px-2.5 py-1 rounded-full">{Math.round(selectedDetailTransaction.categoryConfidence * 100)}%</div>
                                        </div>
                                    )}
                                </div>
                            </motion.div>
